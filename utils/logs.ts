...
export const Log = (response: AxiosResponse) => {
  switch (response.config.method) {
    ...
    case "post": {
      return (
        console.log(`%c POST --> ${response.request.responseURL} \n Data-->`, "background: #222; color: #bada55; font-style: italic"), response.data
      );
    }
...
export const LogError = (error: AxiosError<any>): void => {
  ...
  console.error(
    `%c ERROR --> ${error?.config && error.config.method?.toUpperCase()} --> ${requestUrl}\n Status --> ${status} ${statusText}\n`,
    "background: #222; color: #dc3545; font-style: italic",
